...
class TestDiceMetric:
    ...
    def test_perfect_prediction_dice_is_one(self):
        ...
        assert torch.allclose(dice, torch.ones_like(dice), atol=1e-4), (
    ...
    def test_all_wrong_prediction_dice_is_zero(self):
        ...
        dice = compute_dice(pred, target, threshold=0.5)
        assert torch.allclose(dice, torch.zeros_like(dice), atol=1e-4), (
...
class TestExpectedCalibrationError:
    ...
    def test_perfect_calibration_ece_near_zero(self):
        ...
        ece, _, _, _ = expected_calibration_error(probs, correct.astype(float), n_bins=10)
        assert ece < 0.15, f"ECE should be small for roughly calibrated model, got {ece:.4f}"
...
class TestEntropyAUC:
    ...
    def test_auroc_random_is_near_half(self):
        ...
        auc = entropy_auc(unc, errors)
        if not math.isnan(auc):
            assert 0.30 <= auc <= 0.70, f"Random uncertainty AUROC should be ~0.5, got {auc:.4f}"
...
class TestCohensKappa:
    ...
    def test_kappa_interpretation_substantial(self):
        ...
        assert result["kappa"] > 0.6, f"Expected κ>0.6 for 90% agreement, got {result['kappa']:.3f}"
