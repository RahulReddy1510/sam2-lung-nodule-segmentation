...
class TestMCDropoutMode:
    ...
    def test_model_restored_after_context(self, small_model):
        ...
        assert (
            small_model.training == original_training
    ...
    def test_context_restores_on_exception(self, small_model):
        ...
        try:
            ...
            pass
        assert (
            not small_model.training
        ), "Model should remain in eval mode after exception"
...
class TestMCPredict:
    ...
    def test_variance_increases_with_heterogeneous_dropout(self):
        ...
        from models.mc_dropout import mc_dropout_mode, mc_predict
...
class TestEntropyFromSamples:
    ...
    def test_shape_preserved(self):
        ...
        assert entropy.shape == (
            ...
            32,
            32,
        ), f"Expected (2,1,32,32), got {entropy.shape}"
    ...
    def test_entropy_all_certain(self):
        ...
        entropy = entropy_from_samples(samples)
        assert (
            entropy.max().item() < 0.05
        ), f"Entropy should be near 0, got {entropy.max():.4f}"
    ...
    def test_entropy_max_at_half(self):
        ...
        assert (
            ent_half.mean().item() > ent_certa.mean().item()
...
class TestMCDropoutIntegration:
    ...
    def test_full_eval_loop_synthetic(self):
        ...
        for i, batch in enumerate(loader):
            ...
            msks = batch["mask"]
            mean, var = mc_predict(
                model, imgs, n_samples=3, mc_batch_size=3, sigmoid=True
            )
            dm.update(mean, msks)
